-- just update the table name in the first SELECT (in two places)

WITH x AS (
   SELECT count(*)               AS ct
        , sum(length(t::text))   AS txt_len  -- length in characters
        , 'public.bikes_details'::regclass AS tbl  -- provide (qualified) table name here
   FROM   public.bikes_details t  -- ... and here
   )
, y AS (
